# -*- coding: utf-8 -*-
"""
C03-S04-I05 (意識과 人乃天) 논리적 흐름 및 차용 지점 분석
"""
import pandas as pd
import re
import sys
...
# I05 문단 추출
i05 = df_1924[df_1924['local_id'].str.contains('C03-S04-I05', na=False)].copy()

# 문단 ID 추출
def extract_para_id(local_id):
    match = re.match(r'(C03-S04-I05-P\d+)', str(local_id))
    return match.group(1) if match else None

i05['para_id'] = i05['local_id'].apply(extract_para_id)

print('【C03-S04-I05 意識과 人乃天 - 문단별 내용】')
print('=' * 80)

# 문단별 텍스트 집계
paras = i05.groupby('para_id').agg({
...
i05_refs = df_refs[df_refs['항'] == 'I05'].copy()
...
for _, row in i05_refs.sort_values('1924 문단ID').iterrows():
    tokens = str(row['공통 토큰'])[:50]
    print(f"{row['1924 문단ID']:<25} {row['1915 문단ID']:<15} {row['유사도']:<10.4f} {tokens}...")
...
for pid in sorted(paras['para_id'].unique()):
    ...
    # 해당 문단의 참조쌍
    refs = i05_refs[i05_refs['1924 문단ID'] == pid]
...
for pid in sorted(paras['para_id'].unique()):
    if pid is None:
        continue
    cnt = len(i05_refs[i05_refs['1924 문단ID'] == pid])
