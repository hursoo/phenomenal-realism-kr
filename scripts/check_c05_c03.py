...
# C05 × C03 필터링
pairs = df[(df['chapter_1915'] == 'C05') & (df['section_1924'].str.startswith('C03'))]

print('【C05 × C03 문단 쌍 분석】')
print('=' * 80)
print(f'상위 500개 중 C05 × C03: {len(pairs)}개')
print()

# 1915 C05 장 제목 확인
df_1915 = pd.read_excel('app/data/BK_IT_1915_PR_v1.3.xlsx')
c05_title = df_1915[df_1915['local_id'] == 'C05']['kr_text'].values
if len(c05_title) > 0:
    print(f"【1915 C05 제목】: {c05_title[0]}")

# C05 하위 절 확인
c05_sections = df_1915[df_1915['local_id'].str.match(r'^C05-S\d+$', na=False)][['local_id', 'kr_text']]
if len(c05_sections) > 0:
    print("  하위 절:")
    for _, row in c05_sections.iterrows():
...
for i, (_, row) in enumerate(pairs_above.iterrows(), 1):
    ...
    print(f"  [1915] {row['text_1915'][:150]}...")
    print(f"  [1924] {row['text_1924'][:150]}...")
    print()
