...
def gold_text(path):
    ...
    ⚠️ 정본 파일에는 본문 뒤에 작업 기록이 붙어 있는 경우가 있다 — 정정 이력·명확화
    해소·다음 단계 같은 절들이다(C01이 그렇다). 그것까지 정본으로 세면 분모가 부풀어
    **판독 점수가 실제보다 훨씬 낮게 나온다.** C01에서 정본 2,065자 대 판독 1,032자로
    45%가 찍혔는데, 본문만 세면 1,036자로 판독과 거의 같았다(2026-08-12).

    그래서 `[body]` 다음부터 **다음 `##` 절이 시작되기 전까지**만 본문으로 본다.
    """
