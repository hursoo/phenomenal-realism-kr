# -*- coding: utf-8 -*-
"""
문단 유사도 ≥ 0.1 기준 장-장 평균 히트맵 생성 (v3)
- 노이즈 제외: 공통 토큰이 서수(第一~第十 등)만인 쌍은 제외
"""
import pandas as pd
import numpy as np
...
print("=" * 60)
print("【문단 유사도 ≥ 0.1 기준 장-장 평균 히트맵 생성 (v3)】")
print("【노이즈 제외 버전】")
print("=" * 60)
...
# 7. 노이즈 판별: 공통 토큰이 노이즈만인지 확인
def is_noise_only(set1, set2):
    """공통 토큰이 모두 노이즈 토큰인 경우 True 반환"""
    common = set1 & set2
    if not common:
        return True  # 공통 토큰 없으면 의미 없음
    meaningful = common - NOISE_TOKENS
    return len(meaningful) == 0  # 의미있는 토큰이 없으면 노이즈
...
for _, row1 in para_1915.iterrows():
    for _, row2 in para_1924.iterrows():
        ...
        if sim >= THRESHOLD:
            if is_noise_only(row1['tokens'], row2['tokens']):
                noise_count += 1
            else:
                results.append({
...
print(f"\n유사도 ≥ {THRESHOLD} (원래): {len(results) + noise_count}개")
print(f"  - 노이즈 제외: {noise_count}개")
print(f"  - 실질적 참조: {len(results)}개")
...
sns.heatmap(
    ...
    cbar_kws={'label': '평균 유사도 (노이즈 제외)', 'shrink': 0.5},
    ax=ax,
    vmin=0.1,
    vmax=0.25,
    linewidths=0.5,
...
ax.set_title('문단 유사도 ≥ 0.1 기준 장-장 평균 히트맵\n(노이즈 제외, 흰색 = 유의미한 참조 쌍 없음)', fontsize=16, fontweight='bold', pad=20)
...
print("완료!")
print("\n범례:")
print("  - 색상 셀: 유사도 ≥ 0.1인 문단 쌍이 존재 (노이즈 제외)")
print("  - 흰색 셀: 유사도 ≥ 0.1인 문단 쌍이 없음 또는 노이즈만")
