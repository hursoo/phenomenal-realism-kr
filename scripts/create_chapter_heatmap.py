...
print("=" * 60)
print("【문단 유사도 ≥ 0.1 기준 장-장 평균 히트맵 생성】")
print("=" * 60)
...
chapters_1924 = [f'C{i:02d}' for i in range(1, 7)]  # C01-C06

# 평균 매트릭스
pivot_avg = chapter_stats.pivot(index='chapter_1915', columns='chapter_1924', values='avg_similarity')
pivot_avg = pivot_avg.reindex(index=chapters_1915, columns=chapters_1924, fill_value=0)
...
ax.set_title('문단 유사도 ≥ 0.1 기준 장-장 평균 히트맵\n(유의미한 참조 쌍만의 평균)', fontsize=14, fontweight='bold')

plt.tight_layout()
plt.savefig('images/chapter_heatmap_avg_above_threshold.png', dpi=150, bbox_inches='tight')
