...
이 저장소가 담는 1차 자료는 네 갈래다 — **단행본 둘 · 잡지 둘 · 신문 광고 2건 · 판권지 2건.**
아래 표는 중심이 되는 두 단행본이며, 나머지는 [`docs/01_sources.md`](docs/01_sources.md)에 있다.
...
├── data/
│   ├── 1_raw_data/            원본 PDF · 판권지 · 신문 광고 · 월보 통권18 스캔
...
│   └── analysis/              유사도 산출물
├── scripts/                   재현 코드
├── app/                       Streamlit 발표용 앱 (2026-02)
├── presentations/             발표 산출물 — 날짜별
...
**자료와 절차** — `data/` · `docs/` · `scripts/` · `images/`. 누구나 쓸 수 있게 공개하는 것이고,
이 저장소의 본체다.

**분석과 발표** — `app/` · `presentations/` · `docs/analysis-notes/` · `docs/analysis-reports/`.
**2026년 2월 발표에서 이 자료로 무엇을 했는지의 기록**이다. 그때의 방법(문단 단위 · 문턱 0.1 ·
손 필터링)을 딛고 있어 **현재 방법과 다르다.** 지우지 않는 것은 방법이 바뀐 내력 자체가
기록이기 때문이며, 어느 시기의 것인지는 폴더 이름과 각 문서가 밝힌다.
