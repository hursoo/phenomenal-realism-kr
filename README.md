...
| 찾는 것 | 가는 곳 |
|---|---|
| **두 책의 본문이 필요하다** | [`data/3_corpus/`](data/3_corpus/) · 서지는 [01 원사료](docs/01_sources.md) |
| **잡지(개벽·월보) 본문이 필요하다** | [`wolbo/reading/`](data/5_magazine_sources/wolbo/reading/) — 읽을 수 있는 판 166편 |
| **이 자료를 믿어도 되는지 알고 싶다** | ★ [`wolbo/TRUST.md`](data/5_magazine_sources/wolbo/TRUST.md) — 무엇을 말할 수 있고 무엇을 말할 수 없는가 |
| **수치를 재현하고 싶다** | [04 정규화](docs/04_normalization.md) → [05 유사도](docs/05_similarity.md) → `scripts/` |
| **AI를 어디에 썼는지 보고 싶다** | [06 AI 사용 고지](docs/06_ai_disclosure.md) |
| **원본에서 텍스트까지의 절차가 궁금하다** | 아래 [가공 절차](#가공-절차--한눈에)의 그림 하나로 잡힌다 |

**세 줄 요약.** ①두 단행본과 두 잡지의 전문을 싣되, ②원본에서 분석 텍스트에 이르는 **중간 판을
모두 남기고**, ③어느 대목이 사람의 대조를 거쳤고 어느 대목이 기계 판독인지를 **글자마다
표시**한다.
...
이 저장소가 담는 1차 자료는 네 갈래다.

| 갈래 | 자료 | 규모 | 디지털화 |
|---|---|---|---|
| **단행본** | 『哲學と宗敎』(1915) · 『人乃天-要義-』(1924) | 29장 · 6장 | Gemini 3.0 판독 + 여러 판 교정 |
| **잡지** | 『개벽』 · 『천도교회월보』 | **74편 · 166편** | **개벽은 디지털 원문 · 월보는 3엔진 합의(+영인본 9편은 단일 엔진)** |
| **신문** | 『동아일보』 광고 | 2건 | 지면 이미지 + 문안 전사 |
| **판권지** | 두 단행본 | 2건 | 육안 판독 |

상세는 [`docs/01_sources.md`](docs/01_sources.md), 잡지의 가공 절차는
[`docs/02b_digitization_magazines.md`](docs/02b_digitization_magazines.md).

아래 표는 중심이 되는 두 단행본이다.
...
| | 편수 | 시기 | 출처 | DB |
|---|---:|---|---|---|
| 『개벽』 | 74 | 1920. 6. ~ | 국사편찬위원회 한국사DB **디지털 원문** | `MA_YD_10-20_GB.xlsx` |
| 『천도교회월보』 | 166 | 1911. 1. ~ 1922. 5. | 지면 판독 (3엔진 합의 · 영인본 9편은 단일 엔진) | `MA_YD_10-20_WB.xlsx` |

단행본을 곧바로 맞대는 분석에는 들어가지 않으나, **「없다」는 전수 판정의 사정권**이자 어휘
시계열의 근거가 된다.

🔴 **『개벽』 74편은 필자명 아홉 낱말**(`李敦化`·`夜雷`·`야뢰`·`白頭山人`·`백두산인`·
`滄海居士`·`창해거사`·`猪巖`·`저암`)**로 2,516편에서 걸러낸 것이다.** 이 목록이 코퍼스의
경계를 만든다 — `東洋室主人`이 없어 「今後의 天道敎」가 밖에 있다.

⚠️ **월보 전사는 대부분 미검수 초벌이다.** 166편의 등급은 넷으로 갈린다.

| 등급 | 편수 | 뜻 |
|---|---:|---|
| **정본** | 10 | 사람이 전문을 원본 지면과 대조해 확정 |
| **초벌** | 147 | 판독 엔진 셋을 자동으로 합의시킨 판. 갈린 자리가 편당 수백 곳 남아 있다 |
| **영인·단일** | 8 | 국중 스캔이 없어 영인본을 촬영·판독. **대조 엔진이 없어 갈린 자리가 표시되지 않는다** |
| **본문 없음** | 1 | 통권 18(1912-01). 원본 스캔을 직접 판독해 확인했다 |

**그래서 완성될 때까지 기다리지 않고 미완인 채로 내놓았다.** 대신 **어디가 어느 등급인지를
글자마다 표시**한다. 인용하려면 그 자리를 지면과 대조해야 하며, 이미 대조를 마친 대목은
[`verified_passages.md`](data/5_magazine_sources/wolbo/verified_passages.md)에 있다.

| | |
|---|---|
| [`reading/`](data/5_magazine_sources/wolbo/reading/) | **읽을 수 있는 본문 166편** — 마커를 해소하되 〔규칙〕〔대장〕〔C〕〔G〕〔?…〕〔확인〕으로 해소 방식을 표시. 영인본에서만 얻은 편은 〔영인·단일〕 |
...
| [`verified_transcripts/`](data/5_magazine_sources/wolbo/verified_transcripts/) | 정본과 검수 규약 |

**원본 지면 스캔 800장(160편)이 함께 있다**(`source_pages/`, 국립중앙도서관 스캔). 판독이 갈린
자리에서 어느 쪽이 맞는지 남이 판정할 수 있다. 초벌본의 마커는 34,146이고, 기사 경계 밖(앞 글의 꼬리·다음 글의
머리·면주·캡션)을 걷어낸 뒤 읽는 판에 남는 것이 31,885다. **그 가운데 70.5%가 기계가 한쪽을
고른 자리**이며 5.7%는 아무도 고르지 못한 대분기다. 그 비율을 감추지 않는 것이 이 공개의
요건이다.

📌 **2026-08-13 증보 — 국립중앙도서관에 없던 아홉 편.** 편자 소장 『天道敎會月報 影印本』
(天道敎中央總部, 1980)에서 지면을 찾아 촬영·판독했다. 이로써 1911~1922년 이돈화 글의
사정권이 실질적으로 닫혔고(남은 한 편 통권 134는 영인본에도 지면이 없다), **1916년의 공백도
메워졌다**(통권 77 「人生」). 이제 이 기간에 빈 해가 없다.
...
│   ├── 5_magazine_sources/    잡지의 수집 원본 · 원본 지면 스캔 · 판독 중간 산출
│   ├── 4_tokens/              정규화 토큰
│   └── analysis/              유사도 산출물
├── scripts/                   재현 코드
...
flowchart TD
    subgraph CO["수집 Collect — 물질성"]
        ...
        A3["『개벽』<br/>국사편찬위 한국사DB"]
        A4["『천도교회월보』<br/>지면"]
    end

    subgraph RE["정제 Refine — 구조"]
        B["② 페이지 분할 · 재넘버링<br/>A/C/Z · 832장 · 290장"]
        C["③ 판독<br/>Gemini 3.0"]
        D["④ 교정 · ⑤ 문장 태깅<br/>21판"]
        M1["수집 2,516편<br/>→ 필자명으로 74편 선별"]
        M2["단 분리 → 3엔진 판독<br/>Claude · Gemini · Paddle"]
        M3["합의 → 사람 중재<br/>→ 고충실 전사본"]
        M4["영인본 촬영 → 사람이 단 절단<br/>→ 단일 엔진 판독 (9편)"]
        E["⑥ 문장 분리 · ID 부여<br/>단행본 v1.0/v1.1 · 잡지 MA_*"]
        F["⑦ 정리 · ⑧ 묶음 단위<br/>v1.5 / v1.4"]
        H["⑨ 정규화<br/>reading-space 토큰"]
    end

    subgraph IN["분석 Investigate — 관찰"]
        J["⑩ 유사도 · 전수 검색<br/>자카드 930,680쌍"]
    end
    ...
    class C,D,M2,M3,M4 ai
    class B,M1,E,F,H,J rule
    class K,L interp
```
...
| 자료 | 판독 | 사람의 검수 |
|---|---|---|
| 두 단행본 | **Google Gemini 3.0** — 전문 판독 + 장·절·항 위계 인식 | 여러 판에 걸친 교정(21판) |
| **『개벽』** | **없음** — 국사편찬위 디지털 원문 | — |
| **『천도교회월보』** | **3엔진 합의** — Claude Opus(주) · Gemini 2.5 Pro(보강) · PaddleOCR(검증). 국중 스캔이 없던 **영인본 9편은 단일 엔진**(Claude Opus 5, 대조 없음) | ⚠️ **166편 가운데 전문 대조를 마친 것은 9편**(정본). 영인본 9편과는 **다른 편들**이다 |
| 분석 스크립트 | Claude Code | 요구 명세·검수 |

곧 **『개벽』에는 판독 위험이 없고, 두 단행본은 단일 엔진에 여러 판 교정을 얹었으며, 월보는 세
엔진이 서로를 견제했으나 그 차이의 대부분이 사람 손을 거치지 않았다.** 위험의 형태가 셋 다 다르다.

🔴 **월보 코퍼스는 초벌 위에 서 있다.** 147편은 엔진이 갈린 자리가 편당 수백 곳 남은 채로 들어가
