...
| 텍스트 | 원저자 | 간행년 | 행 수 | 문단 수 | 한자어 토큰 |
|--------|--------|--------|-------|---------|------------|
| 『철학과 종교』 | 이노우에 데츠지로 | 1915 | 11,088 | 626 | 39,938 |
| 『인내천요의』 | 이돈화 | 1924 | 2,254 | 365 | 15,868 |
| **비율 (1915:1924)** | | | **4.9:1** | **1.7:1** | **2.5:1** |

두 텍스트는 한자어 토큰 기준 약 2.5:1의 비대칭성을 보인다. 이러한 비대칭성은 절대빈도 비교를 어렵게 하므로, 본 연구에서는 상대빈도(천분율, ‰)를 함께 활용하였다.
...
**『철학과 종교』(1915)**
- 29개 장(C00~C28: 서장 1개 + 본문 26개 + 부록 2개), 장-문단-문장 체계
- `C##-P##-S##` 형식의 식별자
- 각 장은 독립적인 강연/논문에 해당
...
이 부재는 우연인가, 아니면 의도적 선택인가? 이 질문에 답하기 위해 두 텍스트에서 '哲學'이 차지하는 위상을 정량적으로 비교하였다.
...
전체 참조쌍 데이터는 아래 파일에서 확인할 수 있다:

- **`data/analysis/validated_pairs_final.csv`** — 111개 유효 참조쌍 (순위, 유사도, 문단ID, 공통 토큰 포함)
