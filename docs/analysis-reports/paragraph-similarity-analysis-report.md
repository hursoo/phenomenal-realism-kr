...
자카드 유사도 공식 `|A∩B| / |A∪B|`에서 0.1을 달성하려면:

**예시 1**: 두 문단이 각각 10개의 한자어를 가질 때
- 공통 토큰 2개, 전체 토큰 18개 → 유사도 = 2/18 = **0.11** ✓
- 공통 토큰 1개, 전체 토큰 19개 → 유사도 = 1/19 = **0.05** ✗

**예시 2**: 실제 고유사도 쌍 (Rank 1, 유사도 0.3077)
```
1915 C13-S01-P05: {復活, 基督, 基督敎, 儒敎, 信仰, ...} (약 10개)
1924 C06-S06-P06: {復活, 基督, 基督敎, 儒敎, 來世, 信者, ...} (약 9개)
공통: {復活, 基督, 基督敎, 儒敎} = 4개
합집합: 13개
유사도: 4/13 = 0.3077
```

**해석**: 유사도 0.1 이상은 두 문단이 **2개 이상의 핵심 개념어**를 공유해야 도달 가능하다. '基督敎' 하나만 공유하는 경우는 대부분 0.05 미만에 머문다.
