...
#### 3.3.1 통계적 근거: 평균의 20배

228,490개 쌍의 평균 유사도는 **0.005**이다. 임계값 0.1은 이 평균의 **20배**에 해당하는 값으로, 우연히 발생하기 극히 어려운 수준이다.

```
평균 (μ):       0.005
표준편차 (σ):   0.013
임계값 0.1:     μ + 7.3σ (평균에서 표준편차 7.3배 떨어진 값)
```

정규분포를 가정할 경우, μ+7σ를 초과할 확률은 사실상 0에 가깝다. 실제로 228,490개 중 135개(0.06%)만이 이 기준을 충족하여, 이들이 **통계적 이상치(outlier)**임을 확인할 수 있다.

#### 3.3.2 의미론적 근거: 자카드 유사도 0.1의 실제 의미

**문단 단위 분석의 함의**

본 연구는 개별 문장이 아닌 **문단 단위**로 유사도를 산출하였다. 문단은 문장보다 훨씬 많은 토큰을 포함하므로, 동일한 유사도 값이라도 그 의미가 다르다.

| 텍스트 | 문단 수 | 문단당 평균 한자어 종류 |
|--------|---------|------------------------|
| 1915 철학과 종교 | 626개 | **40.2종** |
| 1924 인내천요의 | 365개 | **31.7종** |

*주: 자카드 유사도는 집합 연산이므로, 중복을 제거한 **고유 단어 종류(types)**를 기준으로 산출한다.*

**0.1 달성에 필요한 공통 토큰 수**

자카드 공식 `J = |A∩B| / |A∪B|`를 정리하면, 유사도 0.1을 달성하기 위해 필요한 공통 토큰 수는 다음과 같이 계산된다:
...
평균적인 문단 크기(1915: 40종, 1924: 32종)에서 유사도 0.1을 달성하려면, **최소 7종의 동일한 한자어**가 두 문단에 공통으로 등장해야 한다.

**직관적 해석**: '宗敎', '世界' 같은 범용어 1~2개만 공유해서는 0.1에 도달할 수 없다. '唯物論', '唯心論', '實在論', '意識', '精神' 같은 **특정 주제의 전문 개념어**가 여러 개 겹쳐야 비로소 0.1에 도달한다. 문장이 아닌 '문단' 단위이므로, **0.1은 상당히 엄격한 기준**이다.
...
이처럼 유사도 0.1 근처에서도 '一神敎', '超人的' 같은 **종교학 핵심 개념어**가 공유되어야 도달 가능하다. 단순한 범용어 공유로는 0.1에 미치지 못한다.

**고유사도 쌍 예시 (Rank 1, 유사도 0.3077)**

```
1915 C13-S01-P05: {復活, 基督, 基督敎, 儒敎, 信仰, ...}
1924 C06-S06-P06: {復活, 基督, 基督敎, 儒敎, 來世, 信者, ...}
공통: {復活, 基督, 基督敎, 儒敎} = 4개
합집합: 13개
유사도: 4/13 = 0.3077
```

**요약**: 유사도 0.1 이상은 두 문단이 **복수의 핵심 개념어**를 공유해야 도달 가능하며, 이는 우연적 일치가 아닌 **의도적 참조의 지표**로 해석할 수 있다.

#### 3.3.3 실용적 근거: 내용 검증 결과

임계값 0.1 이상인 135개 쌍을 전수 검토한 결과:

| 임계값 | 쌍 수 | 내용 검증 결과 |
|--------|-------|----------------|
| 0.05~0.099 | 약 1,000개 | 대부분 '宇宙', '世界' 등 범용어만 공유 |
| **0.10~0.149** | **76개** | 학술 개념어 공유 (24개 노이즈 제외 후 52개 유효) |
| **0.15 이상** | **59개** | 핵심 개념어 다수 공유 (전량 유효) |

0.1 미만 구간은 실질적 참조로 보기 어려운 쌍이 대다수였고, 0.1 이상 구간에서는 **82%**(111/135)가 실질적 참조 관계로 확인되었다.
...
```
예시:
- 1915: "第一に此の問題を論ずるには..."  (첫째로 이 문제를 논하려면...)
- 1924: "第一 宇宙의 根本原理는..."      (첫째 우주의 근본원리는...)
- 공통 토큰: {第一}
- 판정: 노이즈 (항목 번호만 우연히 일치, 내용은 무관)
```

#### 유형 2: 범용어만 공유 (17개)

'如何', '對照', '區別', '宇宙'(단독), '世界'(단독) 등 **일반적 표현만** 공유하고, 주제 특정적 개념어는 공유하지 않는 경우.

```
예시:
- 1915: "如何なる場合に於ても此の區別は..."  (어떤 경우에도 이 구별은...)
- 1924: "如何한 点에서 區別이 生할가..."      (어떤 점에서 구별이 생길까...)
- 공통 토큰: {如何, 區別}
- 판정: 노이즈 (메타적 표현만 일치, 주제 특정적 개념어 없음)
```
...
*그림 1. 장-장 단위 유효 참조쌍 분포 (111개, 한자어 자카드 유사도 ≥ 0.1, 노이즈 24개 제외). 색상은 참조쌍 개수, 괄호 안은 평균 유사도. C03(人乃天과眞理)과 C06(인내천의雜感)에 집중.*
...
| 12 | 0.1818 | C13-S01-P05 | C06-S06-P02 | 基督敎, 儒敎 |
